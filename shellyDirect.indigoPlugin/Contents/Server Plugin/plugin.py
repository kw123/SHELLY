...
####-------------------------------------------------------------------------####
	def __init__(self, pluginId, pluginDisplayName, pluginVersion, pluginPrefs):
		...
		indigo.server.log( "indigo                  {}".format(self.indigoRootPath))
		indigo.server.log( "installFolder           {}".format(self.indigoPath))
		indigo.server.log( "plugin.py               {}".format(self.pathToPlugin))
		indigo.server.log( "Plugin params           {}".format(self.indigoPreferencesPluginDir))
		...
		indigo.server.log( "check                   {}  <<<<    for detailed logging".format(self.PluginLogFile))
		indigo.server.log( "Plugin short Name       {}".format(self.pluginShortName))
		indigo.server.log( "my PID                  {}".format(self.myPID))
		indigo.server.log( "set params for indigo V {}".format(self.indigoVersion))
	...
	def startup(self):
		try:
			if self.pathToPlugin.find("/"+self.pluginName+".indigoPlugin/")==-1:
				...
				self.errorLog("---------------------------------------------------------------------------------------------------------------" )
				self.errorLog("---------------------------------------------------------------------------------------------------------------" )
				self.errorLog("-------  the plugin has stopped, waiting for you to change the name and restart it  ---------------------------" )
				self.errorLog("-------  the plugin has stopped, waiting for you to change the name and restart it  ---------------------------" )
				self.errorLog("-------  the plugin has stopped, waiting for you to change the name and restart it  ---------------------------" )
				self.errorLog("-------  the plugin has stopped, waiting for you to change the name and restart it  ---------------------------" )
				self.errorLog("---------------------------------------------------------------------------------------------------------------" )
			...
			self.indiLOG.log(2, " --V {}   initializing  -- ".format(self.pluginVersion))
			...
			self.indiLOG.log(50, "--------------------------------------------------------------------------------------------------------------")
			self.indiLOG.log(50, "", exc_info=True)
...
####-------------------------------------------------------------------------####
	def setVariables(self):
		try:
			...
			self.unfiCurl					= self.pluginPrefs.get( "unfiCurl", "/usr/bin/curl")
			...
			try:
				self.tempUnits				= self.pluginPrefs.get("tempUnits", "C")[0]
			except:
				self.tempUnits				= "C"
			...
			try: 	self.energyDigits		 = int(self.pluginPrefs.get("energyDigits", 1))
			except:	self.energyDigits		 = 1

			try: 	self.powerDigits		 = int(self.pluginPrefs.get("powerDigits", 1))
			except:	self.powerDigits		 = 1

			try: 	self.voltageDigits		 = int(self.pluginPrefs.get("voltageDigits", 1))
			except:	self.voltageDigits		 = 1

			try: 	self.currentDigits		 = int(self.currentDigits.get("currentDigits", 1))
			except:	self.currentDigits		 = 1

			self.sensorApiVersion			= self.pluginPrefs.get("sensorApiVersion","1")

			self.portOfIndigoServer			= int(self.pluginPrefs.get("portOfIndigoServer","7980"))
			self.portOfShellyDevices		= int(self.pluginPrefs.get("portOfShellyDevices","80"))

			self.userIDOfShellyDevices		= self.pluginPrefs.get("userIDOfShellyDevices", "")
			self.passwordOfShellyDevices	= self.pluginPrefs.get("passwordOfShellyDevices", "")
			self.IndigoServerIPNumber		= self.pluginPrefs.get("IndigoServerIPNumber", "192.168.1.x")

			self.logStateChanges			= self.pluginPrefs.get("logStateChanges", "no")


			self.useCurlOrPymethod				= self.pluginPrefs.get("useCurlOrPymethod", "/usr/bin/curl")
			...
			self.indigoFolderName		= self.pluginPrefs.get("indigoFolderName", "shelly")
...
####-------------------------------------------------------------------------####
	def validatePrefsConfigUi(self, valuesDict):
		...
		self.useCurlOrPymethod	= valuesDict["useCurlOrPymethod"]

		self.setLogfile(valuesDict["logFileActive2"])

		self.logStateChanges	= valuesDict["logStateChanges"]
		...
		self.tempUnits					= valuesDict["tempUnits"]	# C, F, K
		self.tempDigits					= int(valuesDict["tempDigits"])  # 0/1/2
		self.energyDigits				= int(valuesDict["energyDigits"])  # 0/1/2
		self.powerDigits				= int(valuesDict["powerDigits"])  # 0/1/2
		self.voltageDigits				= int(valuesDict["voltageDigits"])  # 0/1/2
		self.currentDigits				= int(valuesDict["currentDigits"])  # 0/1/2

		self.IndigoServerIPNumber 		= valuesDict["IndigoServerIPNumber"]
		self.portOfIndigoServer 		= int(valuesDict["portOfIndigoServer"])
		self.userIDOfShellyDevices 		= valuesDict["userIDOfShellyDevices"]
		self.passwordOfShellyDevices 	= valuesDict["passwordOfShellyDevices"]
		self.sensorApiVersion			= valuesDict["sensorApiVersion"]
...
###-------------------------------------------------------------------------####
	def buttonPrintHelpToLogCALLBACK(self):
		...
		helpText +='Shellytrv:                         thermostat valve for radiators. The hardware only supports C not F  \n'
		helpText +='                                   the plugin can set target temp, position of valve, set boost minutes, enable/select schedules  \n'
		helpText +='                                   setting a valve postion disables target temp   \n'
		helpText +='                                   it is setup as a thermostat w/o cool and fan option  \n'
		helpText +='=========================================================================================   \n'
		helpText +='   \n'
		self.indiLOG.log(20,helpText)
...
####-------------------------------------------------------------------------####
	def filterEmeterDevices(self, filter="", valuesDict=None, typeId=""):
		...
		for devId in self.SHELLY:
			...
			##self.indiLOG.log(10, "forcing push menu {};  {}".format(devId, self.SHELLY[devId]["ipNumber"]) )
...
####-------------------------------------------------------------------------####
	def filterActiveShellyDevicesNotChild(self, filter="", valuesDict=None, typeId="", xx=""):
		xList=[]
		for devId in self.SHELLY:
			if devId != 0:
				if not self.SHELLY[devId]["deviceEnabled"]: continue
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmpUnIgnoreIPNumberCALLBACK(self, valuesDict, typeId=""):
		...
		ignoredIPNumbers = self.pluginPrefs.get("ignoredIPNumbers", "").split(",")
		if ipN in ignoredIPNumbers:
			self.ignoredIPNumbers.remove(ipN)
			self.indiLOG.log(10, "added IP#:{} back to accepted IP numbers,  currently ignored:{}".format(ipN, self.ignoredIPNumbers) )
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmpIgnoreIPNumberCALLBACK(self, valuesDict, typeId=""):
		...
		if ipN not in self.ignoredIPNumbers:
			...
			self.indiLOG.log(10, "added IP#:{} to ignored IP numbers, currently ignored:{}".format(ipN, self.ignoredIPNumbers) )
			self.pluginPrefs["ignoredIPNumbers"] = (",".join(self.ignoredIPNumbers)).strip(",")
		return valuesDict
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmIpNumberSetupCALLBACK(self, valuesDict, typeId):
		...
		if self.isValidIP(ip):
			...
			self.indiLOG.log(10, "added IP#:{} to shelly discovery process".format(ip) )
			valuesDict["MSG"] = "added IP#:{} to shelly discovery process".format(ip)
		else:
			self.indiLOG.log(10, "bad IP:{} for shelly discovery process".format(ip) )
			valuesDict["MSG"] = "bad IP:{} for shelly discovery process".format(ip)
		return valuesDict
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmIpNumberRangeSetupCALLBACK(self, valuesDict, typeId=""):
		...
		if self.isValidIP(ipFrom) and self.isValidIP(ipTo):
			...
			self.indiLOG.log(10, "added IP#s:{} -- {}  to shelly discovery process".format(ipFrom, ipTo) )
		else:
			self.indiLOG.log(10, "bad IP:{} - {} ".format(ipFrom, ipTo) )
		return valuesDict
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmpPuschActionCALLBACK(self, valuesDict, typeId=""):
		...
		valuesDict["MSG"] = "not set: push of config to {}, try cancel first".format(devIdSelect)
		oneOk = False
		for devId in self.SHELLY:
			if devId == 0: continue
			if devId == devIdSelect or devIdSelect ==0:
				self.indiLOG.log(20, "forcing push of config to {};  pushIdActive:{}".format(devId, self.SHELLY[devId]["pushIdActive"]) )
				self.indiLOG.log(10, "...last props params send:\n{}".format(json.dumps(self.SHELLY[devId]["lastSuccessfullConfigPush"], sort_keys=True, indent=2)) )
				if self.SHELLY[devId]["pushIdActive"] in ["enabled","waiting", "active"]:
					if devId not in indigo.devices: continue
					#self.indiLOG.log(20, "forcing push of config to {}; test2 passed ".format(devId) )
					try:	dev = indigo.devices[devId]
					except:	continue
					#self.indiLOG.log(20, "forcing push of config to {}; test3 passed ".format(devId) )
					if dev.enabled:
						...
						self.indiLOG.log(20, "resetting config push to {}-{}; pushIdActive:{}".format(dev.name, dev.id,  self.SHELLY[devId]["pushIdActive"]) )
						oneOk = True
					else:
						valuesDict["MSG"] = "dev not enabled"
		if oneOk:
			valuesDict["MSG"] = "forcing push of config to {}".format(devId)
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmpPuschActionCancelCALLBACK(self, valuesDict, typeId=""):
		...
		for devId in self.SHELLY:
			if devId == 0: continue
			if devId == devIdSelect or devIdSelect ==0:
				self.SHELLY[devId]["lastSuccessfullConfigPush"] = {"started":time.time(),"finished":time.time()}
			...
			self.indiLOG.log(10, "stopping all pushes of config to {}".format(devName) )
...
####-------------------------------------------------------------------------####
	def buttonConfirmconfirmpgetEmeterCvsFileCALLBACK(self, valuesDict, typeId=""):
		...
		if  devIdSelect not in self.SHELLY:
			self.indiLOG.log(10, "bad selection for EM cvs data {}".format(devIdSelect) )
			return valuesDict
		dev = indigo.devices[int(devIdSelect)]
		props = dev.pluginProps
		if not props["isChild"]: return
		page =  "emeter/"+str(int(dev.states["childNo"])-1)+"/em_data.csv"
		self.indiLOG.log(10, "getting EM csv data for {}; with: {}".format(dev.name, page) )
		...
		self.indiLOG.log(10, "csv data from:{}:\n{}".format(dev.name, cvsData) )
		return valuesDict
	...
	def buttonConfirmconfirmpRequestStatusCALLBACK(self, valuesDict, typeId=""):
		try:
			...
			for devId in self.SHELLY:
				...
				if devId == devIdSelect or devIdSelect ==0:
					dev = indigo.devices[devId]
					self.indiLOG.log(10, "sending status request to device:{}".format(dev.name) )
			...
			if devId == devIdSelect or devIdSelect ==0:
				...
				for item in propList:
					if item[0].find("SENDTOSHELLYDEVICE-") > -1:
							...
							out+= "{:33s}:  {}\n".format("SENDTOSHELLYDEVICE-",settings)
					else:
...
####-------------------------------------------------------------------------####
	def triggerEvent(self, eventId):
		if	time.time() < self.currentlyBooting: # no triggering in the first 100+ secs after boot
			#self.indiLOG.log(10, "triggerEvent: %s suppressed due to reboot" % eventId)
			return
	...
	def deviceStartComm(self, dev):
		try:
			if self.decideMyLog("SetupDevices"):
				self.indiLOG.log(10, "deviceStartComm called for {}; dev={}".format(dev.id, dev.name) )
			...
			if dev.deviceTypeId not in _emptyProps:
				self.indiLOG.log(40, "dev: {}  has wrong deviceTypeId: {}; please delete device and restart plugin".format(dev.name, dev.deviceTypeId))
			else:
			...
			self.indiLOG.log(40, "", exc_info=True)
			self.indiLOG.log(40, "for dev: {}".format(dev.name))
		self.doNotrestartDev = ""
		return
		...
		try:
			...
			try:
				if "started" not in self.SHELLY[dev.id]["lastSuccessfullConfigPush"]:
					...
					self.indiLOG.log(20, "setting config push off in renewShelly -1 for devid:{:20}  ".format(dev.id) )
			except Exception:
				self.indiLOG.log(40, "", exc_info=True)
				self.indiLOG.log(20, "resetting config push in renewShelly -2 for devid:{:20}".format(dev.id) )
	...
	def deviceDeleted(self, dev):  ### indigo calls this
		...
		if dev.id in self.SHELLY:
			...
			if 	self.SHELLY[dev.id]["children"] != {}:
				...
				#self.indiLOG.log(10, "devId:{}, children:{}".format(dev.id, children ))
				for devtype in children:
					#self.indiLOG.log(10, "devId:{}, devtype:{}".format(dev.id, devtype ))
		...
		## this must be a child device, delete children entry at parent
			if  self.SHELLY[dev.id]["isChild"]:
				try:
					...
					self.indiLOG.log(20, "", exc_info=True)
...
####-------------------------------------------------------------------------####
	def deviceStopComm(self, dev):
		try:
			if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "deviceStopComm called for dev={}".format(dev.name) )
		except: pass
	...
	def getDeviceConfigUiValues(self, pluginProps, typeId="", devId=""):
		try:
			...
			if devId not in self.SHELLY:
				self.indiLOG.log(30, "the shelly device is new, recommended to use menu / scan to create new shelly devices !!")
	...
	def validateDeviceConfigUi(self, valuesDict, typeId, devId):
		...
		try:
			...
			if not self.isValidIP(ipNumber):
				valuesDict["MSG"] = "ERROR: bad IP Number"
				errorDict["MSG"] = "bad IP Number"
				return ( False, valuesDict, errorDict )

			if not len(MAC):
				valuesDict["MSG"] = "ERROR: bad MAC Number"
				errorDict["MSG"] = "bad MAC Number"
				return ( False, valuesDict, errorDict )
			...
			if devId not in self.SHELLY:
				...
				#errorDict["MSG"] = "use menu/scan to create new shelly devices"
				return ( True, valuesDict, errorDict )


			if MAC != self.SHELLY[devId]["MAC"]:
				self.indiLOG.log(20, " replacing mac#:{} with:{}".format(self.SHELLY[devId]["MAC"], MAC))
				dev = indigo.devices[devId]
			...
			#check if dev type change for type 25 roller <-> relays
			if dev.deviceTypeId.find("shellyswitch25") == 0 and dev.deviceTypeId.find("child") == -1:
				...
				if   dev.deviceTypeId == "shellyswitch25" and 		 valuesDict["SENDTOSHELLYDEVICE-mode"] == "roller":
					...
					self.indiLOG.log(20, "{} changing devtype to roller".format(dev.name))
					devs, devNos = self.getChildDevices(self.SHELLY[devId]["children"])
					for dd in devs:
						try: indigo.devices.delete(dd.id)
						except:pass
					self.SHELLY[devId]["children"] = {}
					self.SHELLY[devId]["isParent"] = False
					...
					self.indiLOG.log(20, "{} changing devtype to 2 relays".format(dev.name))
			...
			if props["isChild"]:
				...
				if newParameters:
					parentDev.replacePluginPropsOnServer(parentProps)
					self.indiLOG.log(20, "resetting config pushstart in validate dev to: {}".format(dev.name))
...
####-------------------------------------------------------------------------####
	def initConcurrentThread(self):
		...
		for ii in range(2):
			if self.pluginPrefs.get("IndigoServerIPNumber","") != "": break
			self.sleep(10)
	...
	def printcProfileStats(self,pri=""):
		try:
			...
			sys.stdout 	= openEncoding(outFile+".txt", "w")
...
####-----------------   main loop          ---------
	def dorunConcurrentThread(self):
		...
		if self.logFileActive !="standard":
			indigo.server.log("..  initialized")
			self.indiLOG.log(10, "..  initialized, starting loop" )
		else:
			indigo.server.log("..  initialized, starting loop ")
				...
				if "devId" in action:
					...
					if action["action"] == "changeDeviceTypeId":
						...
						if action["value"] == "shellyswitch25":
							...
							dev.replacePluginPropsOnServer(props)
						self.indiLOG.log(20, "{} changed devtype to {}, need to edit device again and save".format(dev.name, action["value"]))
						delAction.insert(0,ii)

					if action["action"] == "checkStatus" and time.time() - action["notBefore"] < 0:
						self.addToShellyPollerQueue( action["devId"], "status", now=True, calledFrom="checkForDeviceAction")
						self.indiLOG.log(10, "{} checking status after action".format(dev.name))
						delAction.insert(0,ii)
					...
					if action["action"] == "states" and time.time() - action["notBefore"] < 0:
						...
						self.indiLOG.log(10, "{} adding missing states ".format(dev.name))
						delAction.insert(0,ii)
				ii += 1
		...
		try:
			if self.testHTTPsuccess != 0:
				if time.time() - self.testHTTPsuccess  > 100:
					...
					self.indiLOG.log(10, "HTTPlisten test sucessfull")
				self.testHTTPsuccess = 0
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
		return
...
###-------------------------------------------------------------------------####
	def checkIfPushToDevicesIsRequired(self, now, pushNow=False):
		try:
			#self.indiLOG.log(10, "checkIfPushToDevicesIsRequired dt min {}, push {}".format(time.time() - self.checkTimeIfPushToDevicesIsRequired, pushNow))
			...
			for devId in self.SHELLY:
				...
				try: # just in case not properly defined
					...
					self.SHELLY[devId]["lastSuccessfullConfigPush"] = {"started":-10,"finished":-10}
					self.indiLOG.log(20, "resetting config push in checkIfPushToDevicesIsRequired for devid:{:20}".format(devId) )
				...
				if self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] < 0 or (pushNow and time.time() - self.SHELLY[devId]["lastRequestedPush"] < 10 ):
					if self.SHELLY[devId]["pushIdActive"] in ["enabled","waiting","new"]:
						self.indiLOG.log(10, "checkIfPushToDevicesIsRequired: {}; pushIdActive={}, pushRequest:{}".format(devId, self.SHELLY[devId]["pushIdActive"], self.fromTimeToDate(self.pushRequest)))
	...
	def checkForExpiredDevices(self, now):
		try:
			...
			for devId in delShelly:
				self.indiLOG.log(10, "deleting {} {}".format(devId, self.SHELLY[devId]))
			...
			for ipx in range(rangestart, rangeEnd):
				...
				for devId in self.SHELLY:
					...
					if self.nextIPSCAN == self.SHELLY[devId]["ipNumber"]:
						self.indiLOG.log(10, "SHELLY discovery: skipping ip# {}, alread exists".format(self.nextIPSCAN))
						self.nextIPSCAN = ""
						break
				if self.nextIPSCAN == "": continue
				self.indiLOG.log(10, "SHELLY discovery: probing  ip# {}, for 20 secs".format(self.nextIPSCAN))
				self.SHELLY[0]["ipNumber"] = self.nextIPSCAN
				self.addToShellyPollerQueue(0, "settings", calledFrom="checkForNewScanRequest")
				for ii in range (20):
					time.sleep(1)
					if self.nextIPSCAN == "": break
			self.indiLOG.log(10, "SHELLY discovery: finished")
...
####-------------------------------------------------------------------------####
	def testHTTPlistener(self):
		try:
			...
			if self.decideMyLog("HTTPlistener"): self.indiLOG.log(10, "..  starting http listener TEST @:http://{}:{}/test".format(self.HTTPlisternerTestIP, self.portOfIndigoServer) )
	...
	def workOnQueue(self):
		try:
			...
			while not self.messagesQueue.empty():
				...
				#if self.decideMyLog("Polling"): self.indiLOG.log(10, "workOnQueue  items:{}".format(items))
				for ii in range(40):
...
####-------------------------------------------------------------------------####
	...
	def execUpdate(self, items):
		try:
			if self.decideMyLog("Polling"): self.indiLOG.log(10, "queue received: items:{}".format(items) )
	...
	def workOnRegularMessage(self, items):
		try:
			...
			if ipNumber in self.ignoredIPNumbers:
				if self.decideMyLog("Polling"): self.indiLOG.log(10, "queue checking devid:{};  IP#:{} rejected in ignore list".format( items["shellyIndigoDevNumber"] , ipNumber) )
				return
			...
			if items["shellyIndigoDevNumber"]  == 0:
				...
				if devIdFound > 0:
					if items["shellyIndigoDevNumber"] != devIdFound:  # ip number changed
						...
						self.indiLOG.log(10, "execUpdate dev: {} IPNUMBER has changed ..  new IP#:{}, old IP#:{}".format(devIdFound, ipNumber, self.SHELLY[devIdFound]["ipNumber"]))
				...
				if items["shellyIndigoDevNumber"] == 0:
					for dd in indigo.devices.iter(self.pluginId):
						if dd.address == ipNumber:
							if not dd.enabled:
								self.indiLOG.log(10, "execUpdate shellyIndigoDevNumber ==0, .. dev:{}  is disabled, will NOT create new one;  ipNumber:{}".format(dd.name, ipNumber))
							else:
					...
					if "device" not in data:
						...
						self.addToShellyPollerQueue(0, "settings", now=True, calledFrom="workOnRegularMessage1")
						return
			...
			if items["shellyIndigoDevNumber"]  > 0:
				...
				if "page" in items:
					if items["page"] == "settings":
						...
						self.addToShellyPollerQueue(items["shellyIndigoDevNumber"] , "status", now=True, calledFrom="workOnRegularMessage2")
				if newDeviceCreated:
					self.addToShellyPollerQueue(items["shellyIndigoDevNumber"] , "status", now=True, calledFrom="workOnRegularMessage3")
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
			self.indiLOG.log(40, " data:{}".format(items) )
...
####-------------------------------------------------------------------------####
	def workOnActionMessage(self, items):
		try:
			...
			if self.decideMyLog("HTTPlistener"): self.indiLOG.log(10, "workOnActionMessage queue page item present items:{}".format(items) )
			...
			if ipNumber == self.HTTPlisternerTestIP:
				...
				if self.decideMyLog("HTTPlistener"): self.indiLOG.log(10, "http listener self test received .. ok" )
				return
			...
			if found:
				...
				self.addToShellyPollerQueue(devIdFound, "settings", calledFrom="workOnActionMessage1")
				self.addToShellyPollerQueue(devIdFound, "status", calledFrom="workOnActionMessage2")
			...
			if not found:
				...
				for dd in indigo.devices.iter(self.pluginId):
					if dd.address == ipNumber:
						if not dd.enabled:
							self.indiLOG.log(10, "execUpdate .. dev:{}  is disabled, will NOT create new one; ipNumber:{}, data:".format(dd.name, ipNumber, data))
							doNotCreate = True
						else:
							self.indiLOG.log(10, "execUpdate .. dev:{}  httpAction has unexpected message,from ipNumber:{}, sending status request data:{}".format(dd.name, ipNumber, data))
				if doNotCreate: return
				self.indiLOG.log(10, "execUpdate ..   httpAction has unexpected message,from ipNumber:{}, sending settings request;  data received:{}".format(ipNumber, data))
				self.initShelly(0, "", ipNumber, startPoller=True)
				self.addToShellyPollerQueue(0, "settings", calledFrom="workOnActionMessage3")
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
			self.indiLOG.log(40, " data:{}".format(items) )
		return
...
####-------------------------------------------------------------------------####
	def checkIfExtSensorDevicesnNeedToBeCreated(self, data, parentDev, ipNumber):
		try:
			...
			self.indiLOG.log(10, "external sensors toBeCreated: {}".format(toBeCreated))
			...
			for devTypeId in toBeCreated:
				...
				for nn in toBeCreated[devTypeId]:
					...
					self.indiLOG.log(10, "trying to create new SHELLY device,  name:{}, ipNumber:{}".format((baseName+nameX), ipNumber))
					try:
					...
					self.indiLOG.log(10, "==> created: {}".format(devChild.name))
...
####-------------------------------------------------------------------------####
	def createParentShellyDevice(self, data, ipNumber ):
		try:
			...
			self.indiLOG.log(10, "==> create ShellyDevice for {}, deviceTypeId:{}".format(ipNumber, useDevType) )
			...
			try:
				...
				self.indiLOG.log(10, "==> created: {}".format(devParent.name))
				devId = devParent.id

			except Exception:
				self.indiLOG.log(40, "", exc_info=True)
				self.indiLOG.log(40, "name:{}, props:{}, ipNumber:{}, description:{}, deviceTypeId:>>{}<<\n ".format(name_Parent, props, ipNumber,description, useDevType))
				return 0
	...
	def initShelly(self, dev, MAC, ipNumber, deviceTypeId="", startPoller=True, init=False):
		try:
			...
			if init or "started" not in self.SHELLY[devId]["lastSuccessfullConfigPush"]:
				self.indiLOG.log(20, "resetting config push initShelly for devid:{:15}".format(dev.id) )
				self.SHELLY[devId]["lastSuccessfullConfigPush"] = {"started":-10,"finished":-10}
	...
	def fillShellyDeviceStates(self, data, dev, page, ipNumber):
		...
		try:
			...
			if page == "httpAction":
				...
				self.addToShellyPollerQueue(dev.id, "status", calledFrom="fillShellyDeviceStates")
...
####-------------------------------------------------------------------------####
	def fillHTTPactionData(self, data, dev):
		try:
			...
			if self.decideMyLog("HTTPlistener"):self.indiLOG.log(10, "doHTTPactionData {}, id:{}, devType:{},  data: {}".format(self.SHELLY[dev.id]["ipNumber"],dev.id, dev.deviceTypeId ,data) )
			if dev.deviceTypeId.split("-")[0] not in _definedShellyDeviceswAction: return

			children = self.SHELLY[dev.id]["children"]
			devs, devNos = self.getChildDevices(children)
			devs = [dev] + devs # indexes start w 1 for input ....
			if self.decideMyLog("HTTPlistener"):self.indiLOG.log(10, "doHTTPactionData  .. len(devs):{}, devNos:{}, children:{}".format(len(devs), devNos, children) )
			...
			useDev = dev
			devNo = 0
			if self.decideMyLog("HTTPlistener"):self.indiLOG.log(10, "doHTTPactionData  ..  data after: {}<".format(data) )
			TRIGGERS = []
			if True:
				cmd = data.split("&")
				for item in cmd:
				...
				if self.decideMyLog("HTTPlistener"):self.indiLOG.log(10, "doHTTPactionData {}  devType:{}  TRIGGERS:{}".format(self.SHELLY[dev.id]["ipNumber"], useDev.deviceTypeId , TRIGGERS) )
			else:
				return
			...
			if deviceTypeId in _definedShellyDeviceswAction:
				for trigger in TRIGGERS:
					if self.decideMyLog("HTTPlistener"):						self.indiLOG.log(10, "doHTTPactionData   trigger:{}".format(trigger) )
			...
			# missing or wrong
				...
				self.indiLOG.log(10, "doHTTPactionData {};  not supported message:{}".format(self.SHELLY[dev.id]["ipNumber"], data) )
	...
	def fillbasicProps(self, data, dev, devNo):
		try:
			...
			if "sleep_mode" in data and  "sleepMode" in dev.states:
					...
					xx = "{:d} {:}".format(data["sleep_mode"]["period"],yy)
			...
			if "uptime"   in data and "upFor" in dev.states:
				...
				#self.indiLOG.log(10, "uptime:{:.0f}  upFor:{} M:{}, H:{}, D:{}".format(data["uptime"]/60, upFor, upForM, upForH, upForD ))
				...
				if ("enabled" in data["wifi_sta"] and data["wifi_sta"]["enabled"])  or ("u'connected" in data["wifi_sta"] and data["wifi_sta"]["u'connected"]):
					...
					if self.isValidIP(ipNumber):
						if dev.address != ipNumber:
							if self.decideMyLog("SetupDevices"): self.indiLOG.log(30, "ip number changed for devID{}; old:{} new:{}".format(dev.id, dev.address, ipNumber) )
							props = dev.pluginProps
							props["address"] = ipNumber
							dev.replacePluginPropsOnServer(props)
							renew = True
...
####-------------------------------------------------------------------------####
	def fillLight(self, data, dev):
		try:
			...
			devID = str(dev.id)
			#self.indiLOG.log(10, "fillLight  dev:{}: {}".format(dev.name,data["lights"] ))
			for light in data["lights"]:
				...
				#if self.decideMyLog("Special"): self.indiLOG.log(10, "fillShellyDeviceStates mode:{}, light:{}".format( mode, light) )
				if mode  == "color":
					...
					#if self.decideMyLog("Actions"): self.indiLOG.log(10, "fillShellyDeviceStates setting mode=white, using brigthness, white to {} and bright to :{}".format( int(brightness)*ison, int(brightness)*ison ) )
				...
				elif "brightness" in light:
					if "brightnessLevel" in dev.states: self.addToStatesUpdateDict(devID, "brightnessLevel", int(brightness)*ison, uiValue="{:d}%".format(int(brightness)*ison ))

				if "temp" in light and "whiteTemperature" in dev.states:
					#self.indiLOG.log(10, "fillLight   ...  oldwhiteTemperature:{}: filling w {}".format(dev.states["whiteTemperature"], light["temp"] ))
...
####-------------------------------------------------------------------------####
	def fillshellyMotion(self, data, dev):
		try:
			...
			# for status info
			if "lux" in data:
				self.addToStatesUpdateDict(devID, "lux", 			data["lux"]["value"], "{:d} [Lux]".format(data["lux"]["value"]), decimalPlaces=0)
				self.addToStatesUpdateDict(devID, "illumination", 	data["lux"]["illumination"])
			...
			if "motion" in data:
				...
				if "blind_time_minutes" in data["motion"]:
					xx = "{:d} minutes".format(data["motion"]["blind_time_minutes"])
	...
	def fillSHWT(self, data, dev):
		try:
			...
			if "flood" in data  and "Flood" in dev.states:
				#self.indiLOG.log(10, "flood: regular data:{}".format(data) )
				devID = str(dev.id)
	...
	def fillSHGAS(self, data, dev):
		try:
			...
			if GS["self_test_state"] != "completed":
				#if self.decideMyLog("Special"): self.indiLOG.log(10, "fillSHGAS setting freq to 10 secs" )
			...
			if GS["self_test_state"] == "not_completed" and GS["sensor_state"] == "normal": # startup is finished, but not self test, force selftest
				if "self_test_state" not in self.SHELLY[dev.id] or time.time() - self.SHELLY[dev.id]["self_test_state"] > 100: # only force if last force > 100 secs agao
					...
					self.addToShellyPollerQueue( dev.id, "self_test", now=True, calledFrom="fillSHGAS")
					warmingUp = "starting"
				else:
					warmingUp = "waiting"
			else:
			...
			if GS["self_test_state"] == "completed" and GS["sensor_state"] == "normal":
				if not dev.states["onOffState"]:
					#if self.decideMyLog("Special"): self.indiLOG.log(10, "fillSHGAS setting freq time" )
					if time.time() - self.SHELLY[dev.id]["self_test_state"] < 50:
						#if self.decideMyLog("Special"): self.indiLOG.log(10, "fillSHGAS setting freq to {}".format(float(dev.pluginProps["pollingFrequency"])) )
		...
		try:
			...
			for childDevType in _childDevTypes:
				...
				if childDevType in data and type(data[childDevType]) == type({}) and data[childDevType] !={}:
					...
					#self.indiLOG.log(10, "===fillExternalSensors childDevType data:{}".format(data[childDevType]))
					for ii in data[childDevType]:
						xxx = data[childDevType][ii]
						devNo 	  = ii
						devNoText = str(int(ii)+1)
						#self.indiLOG.log(10, "===fillExternalSensors ii:{}, xxx:{}".format(ii, xxx))
						if _childDevTypes[childDevType]["dataKey"] in xxx:
							state = _childDevTypes[childDevType]["state"]
							#self.indiLOG.log(10, "===fillExternalSensors state :{},  dev-children:{}".format(state,children))
							...
							#self.indiLOG.log(10, "===fillExternalSensors childDevId :{}".format(childDevId))
							if state in childDev.states:
								#self.indiLOG.log(10, "===fillExternalSensors filling sensor w xxx:{}, key:{}, state:{}".format(xxx, _childDevTypes[childDevType]["dataKey"], state))
		...
		try:
			...
			for input in data["inputs"]:
				...
				try: devIDs = str(devs[devNo].id)
					...
					self.indiLOG.log(40, "fillInputs, devNo:{}, len(devs):{}, data {} ".format(devNo,len(devs), data))
					continue
				...
				if "input" in input:
					...
					#self.indiLOG.log(10, "fillInputs, input: devIDs:{};   inp:{}".format(devIDs, inp) )
				...
				if "event" in input and ( not useInput or (useInput and inp == "on") or usesInputForOnOff ):
					for xxx in [["input_short","S","short"], ["input_short_double","SS","short_double"], ["input_short_tripple","SSS","short_tripple"], ["input_long","L","long"], ["input_long_short","LS","long_short"], ["input_short_long","SL","short_long"]]:
						if input["event"] == xxx[1]  and xxx[0] in devs[devNo].states:
							...
							#self.indiLOG.log(10, "fillInputs,event: devIDs:{};   xxx:{}".format(devIDs, xxx) )
							lastEvent = xxx[1]
				...
				if "event_cnt" in input	and  "event_cnt" in devs[devNo].states:
					...
					#self.indiLOG.log(10, "fillInputs, event_cnt: devIDs:{};   evc:{}, uiValue:{}".format(devIDs, input["event_cnt"],uiText) )
...
####-------------------------------------------------------------------------####
	def fillThermostat(self,  data, devs):
		try:
			#thermostates/0:  --> "thermostats":[{"pos":100.0,"target_t":{"enabled":true,"value":31.0,"units":"C"},"tmp":{"value":21.8,"units":"C","is_valid":true},"schedule":false,"schedule_profile":1,"boost_minutes":0}]
	...
	def filleMeters(self,  data, devs):
		try:
			...
			for emeter in data["emeters"]:
				...
				##self.indiLOG.log(10, "devNo {} , ndevs(:{};  dev:{}; emeter:{}".format(devNo, len(devs), devs[devNo].name, emeter))
		...
		try:
			...
			if state.split("_internal")[0] in dev.states or state+"_internal" in dev.states or state in dev.states:
				...
				if state.lower().find("temperature") > -1 or state == "setpointHeat":
					...
					#if dev.id == 1715122746: self.indiLOG.log(10, "fillSensor x:{}, x1:{}, xui:{}, devUnits:{}, tempUnits:{}".format(x , x1, xui, devUnits, self.tempUnits ))
					x = x1
					if state == "Temperature_Target" and "Temperature_Target" in dev.states:
						self.addToStatesUpdateDict(str(dev.id), "Temperature_Target", x, uiValue=xui, decimalPlaces=decimalPlaces)

					elif state == "setpointHeat" and "setpointHeat" in dev.states:
						self.addToStatesUpdateDict(str(dev.id), "setpointHeat", x, uiValue=xui, decimalPlaces=decimalPlaces)

					elif state == "temperatureInput1" and "temperatureInput1" in dev.states:
						self.addToStatesUpdateDict(str(dev.id), "temperatureInput1", x, uiValue=xui, decimalPlaces=decimalPlaces)
					...
					for ttx in _GlobalConst_fillMinMaxStates:
						if ttx in dev.states and ttx+"MaxToday" in dev.states:
							...
							if init: # at start of pgm
								reset = False
								try:
									int(dev.states[ttx+"MinYesterday"])
								except:
									reset = True
								if not reset:
									try:
										if (float(dev.states[ttx+"MaxToday"]) == float(dev.states[ttx+"MinToday"]) and float(dev.states[ttx+"MaxToday"]) == 0.) :  reset = True
									except: pass
								if reset:
		...
		try:
			...
			if stateName in dev.states and stateName+"MaxToday" in dev.states:
				...
				if val > float(dev.states[stateName+"MaxToday"]):
					self.addToStatesUpdateDict(str(dev.id),stateName+"MaxToday",	 val, decimalPlaces=decimalPlaces)
				if val < float(dev.states[stateName+"MinToday"]):
					self.addToStatesUpdateDict(str(dev.id),stateName+"MinToday",	 val, decimalPlaces=decimalPlaces)
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
		return
...
####-------------------------------------------------------------------------####
	def addToShellyPollerQueue(self, shellyIndigoDevNumber, page, now=False, getStatusDelay=0,calledFrom=""):
		try:
			...
			try:
				...
				if self.decideMyLog("Polling"): self.indiLOG.log(10, "addToShellyPollerQueue ip#:{}; page:{}; now:{};  getStatusDelay:{}".format(self.SHELLY[shellyIndigoDevNumber]["ipNumber"], page, now, time.time()-self.SHELLY[shellyIndigoDevNumber]["getStatusDelay"]))
			except Exception:
				self.indiLOG.log(40, "", exc_info=True)
				self.indiLOG.log(10, "addToShellyPollerQueue error for  devid:{} in shelly:{}".format(shellyIndigoDevNumber,self.SHELLY[shellyIndigoDevNumber]))

			#self.indiLOG.log(10, "addToShellyPollerQueue added devid:{} page:{} to queue:{}".format(shellyIndigoDevNumber, page, list(self.SHELLY[shellyIndigoDevNumber]["queue"].queue)))
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
			try:
				devName = indigo.devices[int(shellyIndigoDevNumber)].name
			except:
		...
		try:
			if state =="start":
				...
				for shellyIndigoDevNumber in self.SHELLY:
					...
					try:
						if not self.SHELLY[shellyIndigoDevNumber]["deviceEnabled"]:
							continue
					except Exception:
						self.indiLOG.log(40, "shellyIndigoDevNumber:{};  SHELLY:{}".format(shellyIndigoDevNumber, self.SHELLY), exc_info=True)
						continue

					if shellySelect == "all" or shellyIndigoDevNumber == shellySelect:
							if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "starting UpdateshellyQueues for devId:{} ".format(shellyIndigoDevNumber) )
							if self.SHELLY[shellyIndigoDevNumber]["state"] != "running":
				...
				if (shellySelect == "all" and time.time() - self.laststartUpdateshellyQueues > 70) or shellySelect != "all":
					...
					for shellyIndigoDevNumber in self.SHELLY:
						if self.SHELLY[shellyIndigoDevNumber]["isChild"]: continue
						if not self.SHELLY[shellyIndigoDevNumber]["deviceEnabled"] : continue
						if shellySelect == "all" or shellyIndigoDevNumber == shellySelect:
							...
							if  time.time() - self.SHELLY[shellyIndigoDevNumber]["lastCheck"]> 100:
								...
								self.indiLOG.log(10, "re - starting UpdateshellyQueues for devId:{} ".format(shellySelect) )
								self.startOneShellyDevicePoller(shellyIndigoDevNumber, reason="not running")
		except Exception:
			self.indiLOG.log(40, "self.SHELLY:{}".format(self.SHELLY), exc_info=True)
		return
...
####-------------------------------------------------------------------------####
	def startOneShellyDevicePoller(self, shellyIndigoDevNumber, reason=""):
		try:
			...
			if shellyIndigoDevNumber not in self.SHELLY:
					self.indiLOG.log(10, "no  {} is not in indigo devices, need to create first".format(shellyIndigoDevNumber) )
					return

			if self.SHELLY[shellyIndigoDevNumber]["state"] == "running":
					if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "no need to start Thread, ipnumber {} thread already running".format(self.SHELLY[shellyIndigoDevNumber]["ipNumber"]) )
					return

			if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, ".. (re)starting   thread for ipnumber:{}, state was: {}; reason:{}".format(self.SHELLY[shellyIndigoDevNumber]["ipNumber"], self.SHELLY[shellyIndigoDevNumber]["state"], reason) )
			...
			self.SHELLY[shellyIndigoDevNumber]["pushIdActive"]	= "enabled"
		...
		if shellySelect != "all":
			if shellySelect in self.SHELLY:
				if self.SHELLY[shellySelect]["state"] == "running":
					self.indiLOG.log(10, "stopping Thread, ipnumber {}, devId:{}".format(self.SHELLY[shellySelect]["ipNumber"], shellySelect) )
...
####-------------------------------------------------------------------------####
	def testIfAlreadyInQ(self, page, ipnumber):
		...
		for q in currentQueue:
			if q == page:
				if self.decideMyLog("Polling"): self.indiLOG.log(10, "NOT adding to update list already presend {}".format(next) )
				return True
		return False
...
####-------------------------------------------------------------------------####
	def shellyPollerThread(self, shellyIndigoDevNumber):
		try:
			...
			if self.decideMyLog("Polling"): self.indiLOG.log(10, "shellyPollerThread starting  for devId:{}; ipnumber# {}, threadNumber:{}".format(shellyIndigoDevNumber, self.SHELLY[shellyIndigoDevNumber]["ipNumber"], threadNumber) )
			tries 		= 0
			retryTime 	= .0
			...
			while self.SHELLY[shellyIndigoDevNumber]["state"] == "running":
				...
				if self.SHELLY[shellyIndigoDevNumber]["threadNumber"] != threadNumber:
					if shellyIndigoDevNumber !=0: self.indiLOG.log(10, "shellyPollerThread stopping due to other thread running  for devId:{}; ipnumber# {}".format(shellyIndigoDevNumber, self.SHELLY[shellyIndigoDevNumber]["ipNumber"]) )
					break
				addBack =[]
				ipNumber = self.SHELLY[shellyIndigoDevNumber]["ipNumber"]
				...
				for ii in range(1000):
					...
					try:
						if ipNumber != self.SHELLY[shellyIndigoDevNumber]["ipNumber"]:
							self.indiLOG.log(10, "shellyPollerThread changed IPnumber:{}, continuing w new IP#{};  devId:{}}".format(ipNumber, self.SHELLY[shellyIndigoDevNumber]["ipNumber"], shellyIndigoDevNumber) )
							ipNumber = self.SHELLY[shellyIndigoDevNumber]["ipNumber"]
							break
					except:
					...
					if self.SHELLY[shellyIndigoDevNumber]["now"]:
						...
						####self.indiLOG.log(10, "shellyPollerThread now: true   ipnumber# {}".format( self.SHELLY[shellyIndigoDevNumber]["ipNumber"]) )
						break
				...
				if pollingFrequency == -1:
					pollingFrequency = 10
					if self.SHELLY[shellyIndigoDevNumber]["lastSuccessfullConfigPush"]["finished"] > 0 and time.time() - self.SHELLY[shellyIndigoDevNumber]["lastSuccessfullConfigPush"]["finished"] < self.repeatConfigPush:
				...
				while not self.SHELLY[shellyIndigoDevNumber]["queue"].empty() or (time.time() - lastDefaultRequestTime) > pollingFrequency:
					...
					if self.SHELLY[shellyIndigoDevNumber]["ipNumber"] == "":
						continue

					if self.SHELLY[shellyIndigoDevNumber]["reset"]:
						continue

					#if self.decideMyLog("Polling"): self.indiLOG.log(10, " send to ipNumber:{}  page:{}".format(ipNumber, page) )
					jData , retCode = self.execShellySend(ipNumber, page)
					#if self.decideMyLog("Polling"): self.indiLOG.log(10, "ret page:{}; json:{}".format(page,jData) )
					...
					if retCode == "0": # all ok?
						...
						if tries  > 2 and tries <=20: # wait a little longer
							if self.decideMyLog("Polling"): self.indiLOG.log(10, "shellyPollerThread last querry were not successful wait, then try again; ip:{}".format(ipNumber))
							retryTime = tries+4

						if tries  > 20:
							if self.decideMyLog("Polling"): self.indiLOG.log(10, "shellyPollerThread last querry were not successful  skip; ip:{}".format(ipNumber))
							retryTime = 20
							break
...
####-------------------------------------------------------------------------####
	def execShellySend(self,  ipNumber, page, timeoutMax=10):
		...
		try:
			...
			if pingR != 0:
				if self.decideMyLog("Ping") : self.indiLOG.log(10, " ipnumber{}  does not answer ping - , skipping update".format(ipNumber) )
				return  "ping offline", "pingOffline, timeout"

			#if self.decideMyLog("Polling"): self.indiLOG.log(10, "execShellySend  send to ipNumber:{}  page:{}".format(ipNumber, page) )
...
####-------------------------------------------------------------------------####
	def testPing(self, ipN):
		try:
			...
			if self.decideMyLog("Ping"): self.indiLOG.log(10, " sbin/ping  -c 1 -W 40 -o {}  ret-code: {}".format(ipN, ret) )
...
####-------------------------------------------------------------------------####
	def sendStatusRequest(self, dev):
		try:
			...
			page = "settings"
			if self.decideMyLog("Actions"): self.indiLOG.log(10, "sending to dev {} page={}".format(dev.name, page))
			time.sleep(0.2)
			self.addToShellyPollerQueue( queueID, page, calledFrom="sendStatusRequest1")
			page = "status"
			if self.decideMyLog("Actions"): self.indiLOG.log(10, "sending to dev {} page={}".format(dev.name, page))
			self.addToShellyPollerQueue( queueID, page, calledFrom="sendStatusRequest2")
			return
		except Exception:
			self.indiLOG.log(40, "", exc_info=True)
		return
...
####-------------------------------------------------------------------------####
	def startShelly25CalibrationCALLBACKmenu(self, valuesDict, typeId=""):
		...
		self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="startShelly25CalibrationCALLBACKmenu")
...
####-------------------------------------------------------------------------####
	def startShelly25StopCALLBACKmenu(self, valuesDict, typeId=""):
		...
		queueID	= int(valuesDict["devId"])
		if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending   page:{}".format(queueID, page))
		self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="startShelly25StopCALLBACKmenu")
		self.deviceActionList.append({"devId":int(queueID),"action":"checkStatus","notBefore":time.time() + 20})
		return
...
####-------------------------------------------------------------------------####
	def startShelly25roller_posCALLBACKmenu(self, valuesDict, typeId=""):
		...
		queueID	= int(valuesDict["devId"])
		if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending   page:{}".format(queueID, page))
		self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="startShelly25roller_posCALLBACKmenu")
		self.deviceActionList.append({"devId":int(queueID),"action":"checkStatus","notBefore":time.time() + 20})
		return
...
####-------------------------------------------------------------------------####
	def startShelly25roller_poswTimerCALLBACKmenu(self, valuesDict, typeId=""):
		...
		queueID	= int(valuesDict["devId"])
		if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending   page:{}".format(queueID, page))
		self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="startShelly25roller_poswTimerCALLBACKmenu")
		self.deviceActionList.append({"devId":int(queueID),"action":"checkStatus","notBefore":time.time() + 20})
...
####-------------------------------------------------------------------------####
	def gasAlarmSetCALLBACKmenu(self, valuesDict, typeId=""):
		...
		if  valuesDict["action"] in ["self_test", "mute", "unmute"]:
			...
			if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending   page:{}".format(queueID, page))
			self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="gasAlarmSetCALLBACKmenu")
...
####-------------------------------------------------------------------------####
		#  http://192.168.1.119/thermostats/0/?target_t=20&pos=50&schedule=true&boost_minutes=5&schedule_profile=2
	def thermostatCALLBACKaction(self, action1=None, typeId="", devId=0):
		valuesDict = action1.props
		self.thermostatCALLBACKmenu(valuesDict)
		return
...
####-------------------------------------------------------------------------####
	def thermostatCALLBACKmenu(self, valuesDict, typeId=""):
		...
		queueID	= int(valuesDict["devId"])
		if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending   page:{}".format(queueID, page))
		self.addToShellyPollerQueue( queueID, page, now=True, calledFrom="thermostatCALLBACKmenu")
...
####-------------------------------------------------------------------------####
	def actionControlThermostat(self, action, dev):
		...
		if self.decideMyLog("Actions"): self.indiLOG.log(10, "actionControlThermostat:  {}, type:{}; --  action=\n{}".format(dev.name, dev.deviceTypeId , action))
...
# noinspection SpellCheckingInspection
	def actionControlDimmerRelay(self, action, dev):
		try:
			devId = dev.id
			if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS:  {}, type:{}; --  action=\n{}".format(dev.name, dev.deviceTypeId , action))
			...
			if dev.deviceTypeId not in ["shellyswitch25-roller", "shellytrv-child"] and ( "redLevel" in dev.states or "brightnessLevel" in dev.states or  "whiteLevel" in dev.states  ):
				...
				elif "brightnessLevel" in dev.states:										channel = "brightnessLevel"
				else:																		channel = "white"
				...
				if self.decideMyLog("Actions"): self.indiLOG.log(10, "Channel: {}".format(channel))
				...
				if setAction:
						if self.decideMyLog("Actions"): self.indiLOG.log(10, "Channel: {}, actionValues:{}, IndigoStateMapToShellyDev:{}".format(channel, actionValues, IndigoStateMapToShellyDev))
						ison = False
			...
			if len(page) > 0:
				...
				if setThermometer != {}:
					#self.indiLOG.log(10, "setThermometer  send: {}".format(setThermometer ))
					self.fillLight( setThermometer, dev)
					self.executeUpdateStatesDict()

				if extraPage != "":
					self.addToShellyPollerQueue( queueID, extraPage, now=True, getStatusDelay=time.time()+1, calledFrom="actionControlDimmerRelay1")
					time.sleep(0.2)

				page = page.strip("&")
				page = _emptyProps[dev.deviceTypeId]["setPageActionPageOnShellyDev"][channel]+page
				if self.decideMyLog("Actions"): self.indiLOG.log(10, "ACTIONS: dev {} sending  channel:{};  page:{} deviceTypeId:{}, emptyProps:{}".format(dev.name, channel, page, dev.deviceTypeId, _emptyProps[dev.deviceTypeId]["setPageActionPageOnShellyDev"]))
				now = True if  setThermometer == {} else False
				getStatusDelay = 0 if  setThermometer == {} else time.time()+1
				self.addToShellyPollerQueue( queueID, page, now=True, getStatusDelay=getStatusDelay, calledFrom="actionControlDimmerRelay2")
...
####-------------------------------------------------------------------------####
	def pushThreadLoop(self, devId):
		try:
			...
			#self.indiLOG.log(10, "pushThreadLoop starting for devId:{}, active:{}".format(devId, devId in self.SHELLY))
			...
			while self.SHELLY[devId]["pushstate"] == "running":
				if devId not in indigo.devices:
					...
					continue
				self.sleep(2)
				#if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "pushThreadLoop looping for devId:{}, pushstate:{}, active:{}, lastfinish:{}".format(devId, self.SHELLY[devId]["pushstate"], self.SHELLY[devId]["pushIdActive"], time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"]))

				if self.SHELLY[devId]["pushIdActive"] == "new":
					self.SHELLY[devId]["pushIdActive"] = "active"
					if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "pushThreadLoop adding devId: {} to execPush".format(devId))
	...
	def addToPushConfigToShellyDeviceQueue(self, devId, delay = 0):
		try:
			...
			if self.SHELLY[devId]["pushIdActive"] in ["active","stop"]:
				if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "addToPushConfigToShellyDeviceQueue devId:{} not adding, already in queue".format(devId))
				return
			if time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] < 200:
				if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "addToPushConfigToShellyDeviceQueue devId:{} not adding, just finished {:.0f} secs ago".format(devId,time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"]))
				return
			if time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["started"] < 3600: # redo only after 1 hour
				if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "addToPushConfigToShellyDeviceQueue devId:{} not adding, started {:.0f} secs ago".format(devId,time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"]))
				return
			if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "addToPushConfigToShellyDeviceQueue adding dev:{}, lastfinish:{}".format(dev.name, time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] ) )
			self.SHELLY[devId]["pushIdActive"] = "new"
...
####-------------------------------------------------------------------------####
	def execPush(self, devId, delay = 0):
		try:
			...
			retCode 			= "0"
			self.addToShellyPollerQueue(devId, "settings", calledFrom="execPush")
			self.sleep(1)
			if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "addToPushConfigToShellyDeviceQueue starting for :{}, last pushes:{} ".format(dev.name, self.SHELLY[devId]["lastSuccessfullConfigPush"]) )

			# if sucess full push in the last 5 minutes do not try again
			if self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"]  > 0 and time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"]  < 300: return
			# do not try longer than 5 minutes to push config
			if self.SHELLY[devId]["lastSuccessfullConfigPush"]["started"]   > 0 and time.time() - self.SHELLY[devId]["lastSuccessfullConfigPush"]["started"]   > 300:
				return

			# set starting time stamp for above test
			if self.SHELLY[devId]["lastSuccessfullConfigPush"]["started"] < 0:
				self.SHELLY[devId]["lastSuccessfullConfigPush"]["started"] = time.time()

			retCode = "0"
			for prop in props:
				#self.indiLOG.log(10, "checking for push props devId:{} , prop:{}, props[prop]:{}".format(devId, prop, props[prop]) )
				if prop.find("SENDTOSHELLYDEVICE-") > -1:
					if props[prop] == "none": break
					if prop not in _settingCmds:
						self.indiLOG.log(30, "props devId:{} , prop:{} not defined, check code".format(devId, prop) )
						break
					if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "SENDTOSHELLYDEVICE: {} -> {}".format(props, props[prop] ) )

					for tries in range(2):
						if self.SHELLY[devId]["pushIdActive"] == "stop": 	
							self.indiLOG.log(10, "stopping push for devId:{}  pushIdActive==stop".format(devId, prop, props[prop]) )
							return
						if self.SHELLY[devId]["pushstate"] != "running":	
							self.indiLOG.log(10, "stopping push for devId:{}  state !=running".format(devId) )
							return
						...
						if retCode != "0" and retCode.find("timeout") >-1:
							...
							if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "delaying send by {} secs after {} sends".format(SleepFor, delayCounter) )
						...
						if self.SHELLY[devId]["lastSuccessfullConfigPush"][page]  < 200:
							...
							if udata.find( findItem ) == -1:
								...
								self.indiLOG.log(10, "send config params to devId:{},  answer ok  http://{}/{}<".format(devId, ipNumber, page) )
								break


			if "action_url" in _emptyProps[deviceTypeId] and self.sensorApiVersion in _emptyProps[deviceTypeId]["action_url"]:
						...
						for setting in actionURLs[parameter]:
							...
							if setting == "none":
								if searchFor1 in currentActions and searchFor2 in currentActions:
									...
									if self.decideMyLog("SetupDevices"): self.indiLOG.log(10, "action already set: {};  >{}< >{}< ".format(dev.name, searchFor1, searchFor2) )
									break
							...
							for tries in range(2):
								...
								if  not s1 or not s2: # not successfull
									if udata.find("offline") == -1:
										self.indiLOG.log(10, "send action_url to devId:{},  search1:>>{}<< ok?:{}, search2:>>{}<< ok?:{}\nanswer not ok:>{}<".format(devId, searchFor1, s1, searchFor2, s2, udata) )
									continue
								else:
									self.indiLOG.log(10, "send action_url to devId:{},  answer ok  http://{}/{}".format(devId, ipNumber, page) )
									self.SHELLY[devId]["lastSuccessfullConfigPush"][page] = time.time()
									break
			...
			if self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] == -10:
				...
				for xx in self.SHELLY[devId]["lastSuccessfullConfigPush"]:
					...
					out += xx+"; "
				if out !="":
					self.indiLOG.log(10, "parameters left to push to {}: {}".format(dev.name, out) )
				else:
					self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] = time.time()

			if self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] != -10:
				self.SHELLY[devId]["lastSuccessfullConfigPush"]["finished"] = time.time()
				self.indiLOG.log(20, "successfull parameter push to devId:{}, details in ....Logs/com.karlwachs.shellyDirect/plugin.log file ".format(dev.name) )
				self.pushRequest = -1
...
##############################################################################################
	...
	def getJsonFromDevices(self, ipNumber, page, jsonAction="", port = "", testHTTP=False, noJson=False, timeoutMax=10 ):
		...
		try:
			if self.decideMyLog("Special"): self.indiLOG.log(10, "getJsonFromDevices: ip:{} page:{}".format(ipNumber, page) )
			...
			if self.useCurlOrPymethod.find("curl") > -1:
				...
				if self.decideMyLog("Polling"): self.indiLOG.log(10, "Connection: "+cmdR )
				try:
					...
					try:
						...
						if ipNumber != self.HTTPlisternerTestIP:
							self.indiLOG.log(10, "Shelly curl response from {}  no json object returned: >{}<>{}<".format(ipNumber, ret, retCode) )
						return {}, retCode
					if  jsonAction == "print":
						self.indiLOG.log(10, " Connection  info\n{}".format( json.dumps(jj, sort_keys=True, indent=2)) )
			...
			############# does not work on OSX	el capitan ssl lib too old	##########
				...
				try:
					...
					if testHTTP: return {}, "0"
					self.indiLOG.log(40, "url testHTTP:{}, used: {}".format(testHTTP, url), exc_info=True)
...
####-------------------------------------------------------------------------####
	def executeUpdateStatesDict(self,onlyDevID="0",calledFrom=""):
		try:
			...
			for devId in local:
				#self.indiLOG.log(10, "dev:{}   updating {}".format(devId, local))
				...
				if len(local[devId]) > 0:
					...
					for state in local[devId]:
						if state not in dev.states:
							self.indiLOG.log(40, "executeUpdateStatesDict dev:{} state:{} not present ".format(dev.name, state) )
							continue
						...
						#if devID == 1883741046 : self.indiLOG.log(10, "executeUpdateStatesDict dev:{} state:{} values:{} new:{} ".format(dev.name, state, dev.states[state] , local[devId][state]["value"]) )
						...
						dd = {"key":state, "value":local[devId][state]["value"]}
						...
						if state.find("last") == -1 and state.find("previous") == -1 and state.find("upFor") == -1:
							if self.logStateChanges == "all" or (self.logStateChanges == "onOff" and state == "onOffState") or (self.logStateChanges == "all-noWiFi" and state != "WiFi_rssi"):
								if state+'.ui' in dev.states:
						...
						#if dev.id == 422345573: self.indiLOG.log(10, "adding status dev:{}; state:{} dd:{} to sensorValue".format(dev.name, state, dd ) )
						if  "displaySelect" in props and props["displaySelect"] == state:
							if "SupportsSensorValue" in props and props["SupportsSensorValue"]:
								#if dev.id == 422345573: self.indiLOG.log(10, "adding status dev:{}; added to sensorValue".format(dev.name ) )
								xx = copy.copy(dd)
								xx["key"] = "sensorValue"
								changedOnly.append(xx)
							oneNew = True
					if oneNew and not (props["isRelay"] and onOffStateNotChanged):
						dd = {"key":"lastStatusChange", "value":dateString}
						changedOnly.append(dd)
					#self.indiLOG.log(10, "dev:{} updating states:{} ".format(devId, changedOnly))
...
####-------------------------------------------------------------------------####
	def fromTimeToDate(self, inTime):
		try:
			ret = datetime.datetime.fromtimestamp(inTime).strftime(_defaultDateStampFormat)
		except:
			ret = datetime.datetime.fromtimestamp(time.time()).strftime(_defaultDateStampFormat)
		return ret
...
########################################
	...
	def checkPluginPath(self, pluginName, pathToPlugin):
		...
		if pathToPlugin.find("/" + self.pluginName + ".indigoPlugin/") == -1:
			self.indiLOG.log(50,"--------------------------------------------------------------------------------------------------------------")
			self.indiLOG.log(50,"The pluginName is not correct, please reinstall or rename")
			self.indiLOG.log(50,"It should be   /Libray/....../Plugins/" + pluginName + ".indigoPlugin")
			p = max(0, pathToPlugin.find("/Contents/Server"))
			self.indiLOG.log(50,"It is: " + pathToPlugin[:p])
			self.indiLOG.log(50,"please check your download folder, delete old *.indigoPlugin files or this will happen again during next update")
			self.indiLOG.log(50,"---------------------------------------------------------------------------------------------------------------")
			self.sleep(100)
			return False
		return True
...
####-------------------------------------------------------------------------####
	def readPopen(self, cmd):
		try:
			...
			self.indiLOG.log(40,"{} - {}".format(ret, err), exc_info=True)
...
####-------------------------------------------------------------------------####
	def startHTTPListening(self):
		try:
			self.indiLOG.log(10, "..  starting HTTP listener on port:{} ".format(self.portOfIndigoServer) )
...
class RequestHandler(BaseHTTPRequestHandler):
	def do_HEAD(self):
		if indigo.activePlugin.decideMyLog("HTTPlistener"): indigo.activePlugin.indiLOG.log(10, "RequestHandler  doHead ip{} , path:{}".format(self.client_address, self.path))
		return
	...
	def do_GET(self):
		...
		self.wfile.flush()
		try:
			if indigo.activePlugin.decideMyLog("HTTPlistener"): indigo.activePlugin.indiLOG.log(10, "RequestHandler  do_GET ..  ip{} , path:{}".format(self.client_address, self.path))
			indigo.activePlugin.addtoAnalyzePollerQueue( {"ipNumber":self.client_address[0], "page":"httpAction", "data":{"path": self.path}}  )
		except Exception:
			indigo.activePlugin.indiLOG.log(30, "", exc_info=True)
		return
